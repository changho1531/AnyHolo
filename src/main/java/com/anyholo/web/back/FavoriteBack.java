...
@WebServlet("/Favorite")
public class FavoriteBack extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<jArray.size();i++) {
			...
			memberList.add(new MemberView(new Member(0, obj.get("channelId").toString(), null, null, obj.get("country").toString(), obj.get("searchName").toString(), obj.get("memberName").toString(), obj.get("twitterUrl").toString()),
		...
		request.getRequestDispatcher("/WEB-INF/Favorite.jsp").forward(request, response);
	}
}